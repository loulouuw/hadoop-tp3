...
public class RelationshipRecordReader extends RecordReader<LongWritable, Relationship> {
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        ...
        if (hasNext) {
            ...
            String[] parts = line.split("<->"); // Split on the delimiter

            if (parts.length == 2) {
                currentValue.setId1(parts[0]);
                currentValue.setId2(parts[1].split(";")[0]); // Ignore the timestamp
            } else {
                throw new IOException("Format de l'input invalid: " + line);
            }
        }
        return hasNext;
    }
